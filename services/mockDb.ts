...
class DatabaseService {
  ...
  async updateProfessional(tenantId: string, id: string, pro: Partial<Professional>) {
    try {
      ...
      // role + disableAI + portalPermissions + commissionRate stored in settings JSONB (no schema change needed)
      if (pro.role !== undefined || pro.disableAI !== undefined || (pro as any).portalPermissions !== undefined || (pro as any).commissionRate !== undefined) {
        ...
        if ((pro as any).commissionRate !== undefined) meta[id].commissionRate = (pro as any).commissionRate;
    ...
    try {
      ...
      if (data) {
        ...
        const result: TenantSettings = {
          ...
          asaasPlanId: fu._asaasPlanId ?? '',
          asaasLastPaymentDate: fu._asaasLastPaymentDate ?? '',
          heroImage: fu._heroImage ?? '',
          logoImage: fu._logoImage ?? '',
  ...
  private async _doUpdateSettings(tenantId: string, updates: any) {
    try {
      ...
      const followUpWithMeta = {
        ...
        _aniversarioModes: newS.aniversarioModes ?? curr.aniversarioModes ?? [],
        ...
        _aniversarioSent: newS.aniversarioSent ?? curr.aniversarioSent ?? {},
        ...
        _asaasPlanId: newS.asaasPlanId ?? curr.asaasPlanId ?? '',
        _asaasLastPaymentDate: newS.asaasLastPaymentDate ?? curr.asaasLastPaymentDate ?? '',
        _heroImage: newS.heroImage ?? curr.heroImage ?? '',
        _logoImage: newS.logoImage ?? curr.logoImage ?? '',
