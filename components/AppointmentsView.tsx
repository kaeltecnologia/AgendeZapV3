...
const AppointmentsView: React.FC<{ tenantId: string; onOpenComandas?: () => void; onOpenComandaForAppt?: (apptId: string) => void; defaultProfessionalId?: string; readOnly?: boolean; refreshTicker?: number }> = ({ tenantId, onOpenComandas, onOpenComandaForAppt, defaultProfessionalId, readOnly = false, refreshTicker = 0 }) => {
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      for (const row of bookingRows) {
        ...
        // Comanda em standby criada imediatamente — abre quando cliente chegar
        const svcForComanda = services.find(s => s.id === row.svcId);
        if (svcForComanda) {
          db.createComanda({
            tenant_id: tenantId,
            appointment_id: newApp.id,
            professional_id: row.profId,
            customer_id: customerId,
            items: [{ id: generateId(), type: 'service', itemId: svcForComanda.id, name: svcForComanda.name, qty: 1, unitPrice: svcForComanda.price, discountType: 'value', discount: 0, professionalId: row.profId }],
            status: 'standby',
          }).catch(err => console.error('Erro ao criar comanda standby:', err));
        }
      }
  ...
  const handleFinish = async (statusOverride?: AppointmentStatus) => {
    ...
    // ARRIVED: abre a comanda (standby→open). Se não existe (agend. antigo/IA), cria open.
    if (effectiveStatus === AppointmentStatus.ARRIVED) {
      ...
      try {
        ...
        if (existing) {
          ...
          // se já está open ou closed, não altera
        } else {
          ...
          await db.createComanda({
            tenant_id: tenantId, appointment_id: showFinishModal.id,
            professional_id: showFinishModal.professional_id!,
            customer_id: showFinishModal.customer_id!,
            items, status: 'open',
          });
        }
      } catch (err) { console.error('Erro ao abrir comanda ARRIVED:', err); }
    }

    // FINISHED: fecha comanda (standby, open ou cria se não existe) — regra absoluta
    if (effectiveStatus === AppointmentStatus.FINISHED) {
      try {
        ...
        if (existing && existing.status !== 'closed') {
          ...
          // Agendamento sem comanda (IA/antigo) — cria fechada diretamente
          await db.createComanda({
            tenant_id: tenantId, appointment_id: showFinishModal.id,
            professional_id: showFinishModal.professional_id!,
            customer_id: showFinishModal.customer_id!,
            items: itemsForComanda, status: 'closed', paymentMethod,
            notes: extraNote || undefined, closedAt: now,
            finalAmount: finishDiscount > 0 || extraValue > 0 ? amountPaid : undefined,
          });
        }
        // se existing.status === 'closed' → já finalizada, não sobrescreve
      } catch (err) { console.error('Erro ao fechar comanda:', err); }
    }
                            ...
                            <select
                              ...
                              onChange={async e => {
                                ...
                                if (newStatus === AppointmentStatus.FINISHED) {
                                  ...
                                  // 3. Abrir comanda (standby→open) ou criar se não existe
                                  try {
                                    const svcObj = services.find(s => s.id === a.service_id);
                                    const existingComandas = await db.getComandas(tenantId);
                                    const existingComanda = existingComandas.find(c => c.appointment_id === a.id);
                                    if (existingComanda) {
                                      if (existingComanda.status === 'standby') await db.updateComanda(existingComanda.id, { status: 'open' });
                                    } else {
                                      await db.createComanda({
                                        tenant_id: tenantId, appointment_id: a.id,
                                        professional_id: a.professional_id, customer_id: a.customer_id,
                                        items: svcObj ? [{ id: generateId(), type: 'service' as const, itemId: svcObj.id, name: svcObj.name, qty: 1, unitPrice: svcObj.price, discountType: 'value' as const, discount: 0, professionalId: a.professional_id }] : [],
                                        status: 'open',
                                      });
                                    }
                                  } catch (err) {
